 ...
 * @param {object} _ contains the settings
 * @return {string} command - that was built out
 */
methods.fillCommands = function(_) {
    ...
    if (_.remote || _.source) {
        if (!_.source) {
            console.log(_.error('You need to provide a source to '+
                                'import from'));
            process.exit(0);
        }
        if (!_.remote) {
            console.log(_.error('You need to provide a remote source to '+
                                'import into'));
            process.exit(0);
        }
        console.log(_.inform('You are about to sync from ' + _.source + ' to '+
                            _.remote + '. You should cancel now if that is ' +
                            'not what you want to do!'));

        return methods.fillSyncServerCommand(_);
    }
...
/**
 * Fill Sync Server Command
 * @use construct a source/remote command to that syncs on db to
 *      another - both remote
 * @param {object} _ contains the settings
 * @return {string} command - that was built out
 */
methods.fillSyncServerCommand = function(_) {
    //need two distinct environments
