...
methods.fillCommands = function(_) {
    // use a defined mysql path or the default
    var mysqlPath = _.config.mysqlPath || _.mysqlPath;
    var sshCmd = 'ssh '+ _.verbose + _.ssh + ' mysqldump '+
        '--default-character-set=utf8 --hex-blob '+ _.verbose;
    var mysqlCmd = mysqlPath + 'mysql '+ _.verbose + '--protocol=TCP --port=' +
        _.port + ' ';
    if (_.multiple === undefined) {
        ...
        _.importCmd = mysqlCmd + '-h '+ _.connection.hostname +
           ' -u '+ _.connection.username +' -p"'+ _.connection.password + '" ' +
           _.connection.database;
    ...
    if (_.multiple) {
        ...
        _.importCmd = mysqlCmd + '-h '+ _.connection.local.hostname +
